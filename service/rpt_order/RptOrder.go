...
type RptOrderList struct {
	DB        *sql.DB
	Level     int
	Total     int        `json:"total"`
	RptOrders []RptOrder `json:"RptOrder"`
}
...
func New(db *sql.DB, level int) *RptOrderList {
	if db == nil {
		log.Println(SQL_SELECT, "Database is nil")
		return nil
	}
	return &RptOrderList{DB: db, Total: 0, RptOrders: make([]RptOrder, 0), Level: level}
}
...
func NewUrl(url string, level int) *RptOrderList {
	var err error
...
func (r *RptOrderList) GetTotal(s Search) (int, error) {
	var where string
	l := time.Now()
...
func (r RptOrderList) Get(s Search) (*RptOrder, error) {
	var where string
	l := time.Now()
...
func (r *RptOrderList) GetList(s Search) ([]RptOrder, error) {
	var where string
	l := time.Now()
	...
	for rows.Next() {
		...
		r.RptOrders = append(r.RptOrders, p)
	}
	log.Println(SQL_ELAPSED, r)
	if r.Level == DEBUG {
		log.Println(SQL_ELAPSED, time.Since(l))
	}
	return r.RptOrders, nil
}
...
func (r *RptOrderList) GetExt(s Search) (map[string]string, error) {
	var where string
	l := time.Now()
...
func (r RptOrderList) Insert(p RptOrder) error {
	l := time.Now()
...
func (r RptOrderList) InsertEntity(p RptOrder, tr *sql.Tx) error {
	l := time.Now()
...
func (r RptOrderList) InsertMap(m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	var colNames, colTags string
	valSlice := make([]interface{}, 0)
	for k, v := range m {
...
func (r RptOrderList) UpdataEntity(keyNo string, p RptOrder, tr *sql.Tx) error {
	l := time.Now()
	var colNames string
	valSlice := make([]interface{}, 0)
...
func (r RptOrderList) UpdateMap(keyNo string, m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
...
func (r RptOrderList) Delete(keyNo string, tr *sql.Tx) error {
	l := time.Now()
